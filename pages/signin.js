...
const SignIn = () => {
  ...
  const login = async (values) => {
    ...
    try {
      ...
      await Axios.post(`auth/callback/credentials`, {...values, csrfToken:token, userType:'tenant'})
      const session = await getSession()
      if(session){
        window.location.assign(`${process.env.NEXT_PUBLIC_APP_URL}/dashboard/tenant`);
      }else{
