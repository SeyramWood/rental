...
const ManageUsers = ({ users }) => {
  ...
  const addUser = (values) => {
    ...
    axios
      .post("/api/admins/register", values)
      .then(({data}) => {
        closeAddUserModal();
  ...
  const editUser = (values) => {
    setIsSubmitting(true);
    axios
      .put(`/api/admins/edit/${values.id}`, {firstName:values.firstName, username:values.username})
      .then(({data}) => {
        closeEditUserModal();
        setIsSubmitting(false);
        notification.success({
        ...
        setAllUsers((users) => users = users.map((user) => {
          if(user._id === data.admin._id){
            return {...data.admin, firstName:values.firstName, username:values.username, key: data.admin._id}
          }
          return user
        }))
      })
...
const deleteUser = (user) => {
    axios
      .delete(`/api/admins/delete/${user._id}`)
      .then(({data}) => {
        notification.success({
  ...
  const dashboardLinks = [
    {url:'/dashboard/admin/landlords', label:'Landlords'},
    {url:'/dashboard/admin/tenants', label:'Tenants'},
    {url:'/dashboard/admin/users', label:'Users'},
   ]
 
return(
<Dashboard pageHeader="Manage Users" links={dashboardLinks}>
