...
const ManageUsers = ({ landlords }) => {
  ...
  const addUser = (values) => {
    ...
    axios
      .post("/api/landlords/register", values)
      .then(({data}) => {
        closeAddUserModal();
        ...
        setAllLandlords(users => users = [{...data, key: data._id},...users])
      })
      .catch((err) => {
        setIsSubmitting(false);
        if (err["response"]) {
  ...
  const editUser = (values) => {
    setIsSubmitting(true);
    axios
      .put(`/api/landlords/edit/${values.id}`, {fullName:values.fullName, username:values.username})
      .then(({data}) => {
        closeEditUserModal();
        setIsSubmitting(false);
        notification.success({
        ...
        setAllLandlords((landlords) => landlords = landlords.map((landlord) => {
...
return(
<Dashboard pageHeader="Manage Landlords" links={dashboardLinks}>
      ...
      >
        ...
        <Form.Item
          ...
          rules={[
            {
              required: true,
              message: "Please input landlord phone number!",
            },
          ]}
        >
          <Input />
...
export async function getServerSideProps(ctx) {
    let landlords = [];
    try {
      let { data } = await Axios.get("/landlords");
      if (data && data.length) {
        landlords = data.map((landlord) => {
          return { ...landlord, key: landlord._id };
        });
      } else {
        landlords = [];
      }
      ...
      throw Error("Could not fetch landlords");
    }
    return {
      props: { landlords },
    };
